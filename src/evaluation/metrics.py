...
def evaluate_metrics(y_true_scaled: np.ndarray, y_pred_scaled: np.ndarray, scaler) -> dict:
    ...
    # Step 1: Inverse transform the standardization (Z-score)
    y_true_unscaled_log_reshaped = scaler.inverse_transform(y_true_reshaped)
    y_pred_unscaled_log_reshaped = scaler.inverse_transform(y_pred_reshaped)
    
    # Reshape back to original dimensions (still in log space)
    y_true_unscaled_log = y_true_unscaled_log_reshaped.reshape(original_true_shape)
    y_pred_unscaled_log = y_pred_unscaled_log_reshaped.reshape(original_pred_shape)
    
    # --- START MODIFICATION 1.1.1 (CRITICAL FIX for Log Transform) ---
    # REASON: The pre-processing step now includes a log1p transform.
    #         To get metrics in the original physical units (TECU), we must
    #         apply the inverse transform (expm1) before any metric calculation.
    y_true_unscaled = np.expm1(y_true_unscaled_log)
    y_pred_unscaled = np.expm1(y_pred_unscaled_log)
