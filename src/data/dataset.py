...
class SlidingWindowSamplerDataset(Dataset):
    ...
    def __init__(self, data_path: str, mode: str, L_in: int = 336, L_out: int = 12, stride: int = 1):
        ...
        if self.num_samples <= 0:
            logging.warning(
                f"Insufficient data for windowing: Total length={len(self.X)}, L_in={self.L_in}, L_out={self.L_out}, stride={self.stride}. Resulting samples: {self.num_samples}"
            )
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # 直接返回，不再有任何scaler操作
        return {"x": x_window, "y": target_y, "x_time_features": time_features_window}  # 已是tensor且已标准化  # 已是tensor且已标准化  # 已是tensor
