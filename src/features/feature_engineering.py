...
def create_features_and_targets(file_paths: list, horizon: int = 12) -> dict:
    ...
    for split_name, data in data_splits.items():
        ...
        # REASON: Applying a log transform (log1p for numerical stability) helps
        #         to handle the heavy-tailed distribution of TEC data, making it
        #         more amenable to standardization and modeling.
        tec_data = data['tec']
        tec_data_transformed = np.log1p(tec_data)
        # Use the transformed data for feature and target construction
        feature_tensor_X = construct_feature_tensor_X(tec_data_transformed, broadcasted)
        target_tensor_Y = construct_target_tensor_Y(tec_data_transformed, horizon)
